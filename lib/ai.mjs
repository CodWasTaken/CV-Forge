...
import { profileSchema, websiteSchema } from './schema.mjs';
import { normalizeWebsite } from '../public/lib/website.mjs';
...
export async function handleAiWebsite(req, res, context) {
  if (!context.allowRequest(req, 8)) {
    return json(res, 429, { error: 'Too many AI requests. Please wait a moment and try again.' });
  }
  ...
  const story = typeof body.story === 'string' ? body.story.trim() : '';
  const targetRole = typeof body.targetRole === 'string' ? body.targetRole.trim() : '';
  const currentWebsite = body.website && typeof body.website === 'object' ? body.website : {};
  if (!profile?.personal && !story) return json(res, 400, { error: 'Build or enter your CV details before generating a website.' });

  let result;
  try {
    result = await callCompatibleProvider(
      provider,
      buildWebsiteInstructions({ targetRole }),
      buildWebsitePrompt({ profile, story, targetRole, currentWebsite }),
      context.timeoutMs,
      websiteSchema,
      'website_cv'
    );
  ...
  try {
    ...
    return json(res, 502, { error: `The provider returned malformed website data: ${error.message}` });
  }
  if (!parsed?.website || !Array.isArray(parsed?.coachNotes) || !Array.isArray(parsed?.followUpQuestions)) {
    return json(res, 502, { error: 'The provider response did not match the required website structure.' });
  }

  return json(res, 200, {
    website: normalizeWebsite(parsed.website, profile),
...
function buildWebsiteInstructions({ targetRole }) {
  return [
    'You are an expert portfolio strategist and web copywriter.',
    'Create a modern one-page portfolio/CV website plan from the verified profile data.',
    'The application, not you, will generate the HTML, CSS, and JavaScript. Return only structured content and presentation choices.',
    'Never invent employers, projects, dates, metrics, skills, links, qualifications, or contact details.',
    'Keep the voice confident, specific, concise, and human. Avoid clichés and generic claims.',
    'Featured experience and project values are zero-based indexes into the supplied arrays. Only select indexes that exist.',
    'Choose a theme that matches the profile: aurora is expressive/dark, studio is warm/editorial, mono is minimal/technical.',
    'Use a valid six-digit hex accent color with sufficient contrast.',
    targetRole ? `Emphasize relevance to this target role without changing facts: ${targetRole}` : '',
    'Return exactly one JSON object and no markdown.',
    `The JSON must match this schema: ${JSON.stringify(websiteSchema)}`
  ].filter(Boolean).join('\n');
}

function buildWebsitePrompt({ profile, story, targetRole, currentWebsite }) {
  return [
    `TARGET ROLE: ${targetRole || 'Not specified'}`,
    'VERIFIED PROFILE JSON:', JSON.stringify(profile),
    'SOURCE NOTES:', story || 'No additional notes.',
    'CURRENT WEBSITE SETTINGS:', JSON.stringify(currentWebsite)
  ].join('\n\n');
}
...
async function callCompatibleProvider(provider, instructions, userPrompt, timeoutMs, schema, schemaName) {
  const styles = provider.apiStyle === 'auto' ? ['responses', 'chat_completions'] : [provider.apiStyle];
  const failures = [];
  for (const style of styles) {
    try {
      return await invokeProvider(provider, style, instructions, userPrompt, true, timeoutMs, schema, schemaName);
    } catch (error) {
    ...
    try {
      return await invokeProvider(provider, style, instructions, userPrompt, false, timeoutMs, schema, schemaName);
    } catch (error) {
...
async function invokeProvider(provider, style, instructions, userPrompt, structuredOutput, timeoutMs, schema, schemaName) {
  ...
  const requestBody = style === 'responses'
    ? {
        ...
        instructions,
        input: userPrompt,
        ...(structuredOutput ? { text: { format: { type: 'json_schema', name: schemaName, strict: true, schema } } } : {})
      }
    : {
        model: provider.model,
        messages: [{ role: 'system', content: instructions }, { role: 'user', content: userPrompt }],
        ...(structuredOutput ? { response_format: { type: 'json_schema', json_schema: { name: schemaName, strict: true, schema } } } : {})
      };
